...
* Strings: The " and \` words are powerful immediate 1 string readers. When encountered, they consume all characters from the source stream until a matching closing delimiter is found. They can process escape sequences (e.g.,\n, \t) and return a function that, when executed at runtime, pushes the resulting string onto the stack. This mechanism is more robust than the simple 'prefix handled bycompile_atom`.
